...
public:
	...
	jMockUp(Mockup mockup) {
		...
		for (int indeks = 0; indeks < CARDS; ++indeks) {
			p1Table[indeks].first =
					mockup.cards_on_table_player_A[indeks].getIdOfCard();
			p1Table[indeks].second =
					mockup.cards_on_table_player_A[indeks].getTypeOfCard();
			p2Table[indeks].first =
					mockup.cards_on_table_player_B[indeks].getIdOfCard();
			p2Table[indeks].second =
					mockup.cards_on_table_player_B[indeks].getTypeOfCard();
			p1Hand[indeks].first =
					mockup.cards_in_hand_player_A[indeks].getIdOfCard();
			p1Hand[indeks].second =
					mockup.cards_in_hand_player_A[indeks].getTypeOfCard();
			p2Hand[indeks].first =
					mockup.cards_in_hand_player_A[indeks].getIdOfCard();
			p2Hand[indeks].second =
					mockup.cards_in_hand_player_A[indeks].getTypeOfCard();
		}
	}
	;
